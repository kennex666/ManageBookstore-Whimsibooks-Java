...
public class Form_TraHang extends javax.swing.JFrame {
	...
	public Form_TraHang(HoaDon x, JFrame y, TAB_BanHang tab, HoaDonTra z) {
		...
		String[] options = { "Sản phẩm lỗi do NSX", "Sản phẩm hết hạn", "Khách hàng đổi ý", "KH không hài lòng",
				"Không có lý do" };
	...
	private void initComponents() {
		...
		jTable1.setModel(tblModelTraHang = new javax.swing.table.DefaultTableModel(new Object[][] {},
	...
	private void btnThanhToanHoanTatActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btnThanhToanHoanTatActionPerformed
		...
		if (hoaDonTra.getListChiTietHoaDon().size() == 0) {
            JOptionPane.showMessageDialog(null, "Vui lòng chọn ít nhất 1 sản phẩm trả hàng.");
            return;
        }
		...
		temp.setNhanVien(CurrentSession.getNhanVien());
		temp.setTrangThai(hoaDon.getTrangThai());
		temp.setTongTien(hoaDon.getTongTien());
		temp.setThue(hoaDon.getThue());
		temp.setNgayLapHoaDon(hoaDonTra.getNgayTraHoaDon());

		for (ChiTietHoaDon cthd : hoaDon.getListChiTietHoaDon()) {
			temp.addChiTietHoaDon(new ChiTietHoaDon(cthd.getSanPham(), cthd.getSoLuong()));
		}

		// Phục hồi hoá đơn gốc
		for (ChiTietTraHang cthd : hoaDonTra.getListChiTietHoaDon()) {
			hoaDon.addChiTietHoaDon(new ChiTietHoaDon(cthd.getSanPham(), cthd.getSoLuong()));
		}
		hoaDon.setTrangThai("TRA_HANG");
		hoaDon.setTongTien(hoaDon.tinhTongTien());
//		
//		if (!hoaDon_BUS.cancelHoaDon(hoaDon)) {
//			JOptionPane.showMessageDialog(null, "Đã xảy ra lỗi khi cập nhật trạng thái.");
//			return;
//		}
		...
		result = chiTietTraHang_BUS
				.addNhieuChiTietCuaMotHoaDon((List<ChiTietTraHang>) hoaDonTra.getListChiTietHoaDon());
		if (!result) {
		...
		if (temp.getListChiTietHoaDon().size() > 0) {

			temp.setTrangThai("DA_XU_LY");
			result = hoaDon_BUS.createHoaDon(temp);
			...
			result = chiTietHoaDon_BUS.addNhieuChiTietCuaMotHoaDon((List) temp.getListChiTietHoaDon());
			if (!result) {
